...
std::list<int> SplicingGraph::getParents(const int& i, const int& id_gene) const {
    return vector_gene[id_gene -1 ].parents_g[i];
}

std::list<int> SplicingGraph::getSons(const int& i, const int& id_gene) const {
    return vector_gene[id_gene -1 ].sons_g[i];
}
...
int SplicingGraph::rank_sg(const int& i) const{
    return rankBV_sg(i);
}
int SplicingGraph::select_sg(const int& i) const{
    return selectBV_sg(i);
}
...
bool SplicingGraph::contain(const int& id_gene, const int& x, const int& y) const {
    if(vector_gene[id_gene -1].edges_g[x][y] >= 1) {
        return true;
    }
    return false;
}

bool SplicingGraph::isNew(const int& id_gene, const int& x, const int& y) const {
    if(vector_gene[id_gene -1].edges_g[x][y] > 1) {
        return true;
    }
    return false;
...
int SplicingGraph::getGeneNumber() const {
    return vector_gene.size();
}
