...
std::pair<bool, int> MemsGraph::checkMEMs(const SplicingGraph& sg,
    ...
    int id_gene = sg.rank_sg(m1.t);
/* trovoil rank dei mem su il rank complessivo BV*/
    int id_bv_1 = sg.rank(m1.t-1);
    int id_bv_2 = sg.rank(m2.t-1);

    std::string exon1_text = sg.getExon(id_bv_1);
    std::string exon2_text = sg.getExon(id_bv_2);
    ...
    if(id_bv_1 == id_bv_2) { //m1 and m2 in the same exon
        ...
        if(m2.p+m2.l>m1.p+m1.l && m1.t<m2.t && m1.t+m1.l<m2.t+m2.l) {
            ...
            if(gapP>=0 && gap_E>=0) {
                if(gapP == 0) {
                    ...
                    std::string sub_E = exon1_text.substr(m1.t+m1.l-sg.select(id_bv_1)-1-1, m2.t-m1.t-m1.l);
        ...
        int pos_ = 0;
        if(id_gene > 1){
            pos_ = sg.rank(sg.select_sg(id_gene)+1);
        }
        int id_x_1 = id_bv_1 - pos_ ;
        int id_x_2 = id_bv_2 - pos_ ;

        if(sg.contain(id_gene ,id_x_1, id_x_2)) {
            if(verbose) {
                std::cout << "different exons" << std::endl;
            }
            if(m2.p+m2.l>m1.p+m1.l) {
                int gapP = m2.p-m1.p-m1.l;
                int gapE1 = sg.select(id_bv_1+1)+1-m1.t-m1.l;
                int gapE2 = m2.t-sg.select(id_bv_2)-1-1;
                if(gapP <= 0) {
                    err = 0; //abs(gapP);
                    if(verbose) {
                        std::cout << id_x_1 << " " << id_x_2 << " " << sg.isNew(id_gene ,id_x_1, id_x_2) << " " << gapE1 << " " << gapE2 << std::endl;
                    }
                    if(!sg.isNew(id_gene,id_x_1, id_x_2) && gapE1 == 0 && gapE2 == 0) {
                        type = true;
                    }
                    else if(err <= K2) {
                        //Possible Competing
                    ...
                    if(gapE1 == 0 && gapE2 == 0) {
                        ...
                        if(!sg.isNew(id_gene ,id_x_1,id_x_2)) {
                            err = 0;
                            type = false;
                        }
                    } else {
                        if(abs(gapP-(gapE1+gapE2)) <= K2) {
                            ...
                            std::string subE1 = exon1_text.substr(m1.t+m1.l-sg.select(id_bv_1)-1-1, gapE1);
    ...
    if(Mem.p <= K0) {
        ...
        if(Mem.p == 1) {
            ...
            if(exon_pref_len < l) {
                ...
                err = l;
                std::list<int> parents = sg.getParents(id_x, id_gene);
               ...
               // std::cout << Mem.t << " " << l << " "<< sg.select(id) << std::endl;
...
std::pair<bool, int> MemsGraph::validEnd(const SplicingGraph& sg, const Mem& Mem) {
    if(Mem.p+Mem.l>=m-K0) {
        ...
        if(Mem.p+Mem.l == m+1) {
            ...
            if(exon_suff_len < l) {
                std::list<int> sons = sg.getSons(id_x, id_gene);
...
void MemsGraph::build(const SplicingGraph& sg,
    ...
    for(std::vector<std::forward_list<Mem> >::iterator it=MEMs.begin(); it!=MEMs.end(); ++it) {
        for(Mem& m1 : *it) {
            ...
            std::string exon1_text;

            exon1_text = sg.getExon(id1);
